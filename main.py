...
def profileHome():
    ...
    conn.close()
    return render_template("profileHome.html", loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems,profileData=profileData)
...
def profileOrders():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        profileData = cur.fetchone()
    conn.close()
    return render_template("orders.html", loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems,profileData=profileData)
...
def adminlogin():
    if 'admin' in session:
        return render_template('adminDashboard.html', error='')
    else:
